...
Repository for REST API that provides the car pooling service.

# Technical documentation

- [Car Pooling Service Challenge](#car-pooling-service-challenge)
...
- [ORIGINAL CHALLENGE](#original-challenge)
  ...
  - [API](#api)
    ...
    - [PUT /cars](#put-cars)
    - [POST /journey](#post-journey)
    - [POST /dropoff](#post-dropoff)
...
A API REST is developed to simulate the behavior of car sharing [See challenge](#original-challenge)

The main functions of the API are:

- Add cars to the service resetting all previous values.
- Add journeys to the service, assigning a group of passengers to an available car.
- Drop off the group of people, freeing the car.
- Locate a group of passengers, so that it is possible to know in which car a group is traveling or traveled.

### Endpoints
- `/status` : function that returns OK if the service is already available..
- `/cars` : function that adds cars and resets the state of the service.
- `/journey` : function that tries to assign a car to a group of passengers, creating the corresponding journey. If assigned, it returns the car.
- `/dropoff` : function that removes a group of passengers from the car in which they are traveling.
- `/locate` : function that returns the car in which a group is traveling or has traveled.

## CODE

#### Node JS

Node 14 is required for TypeScript to work, its compiler is developed in Node JS.
Through NPM we maintain all the necessary components to develop and use in production and staging in the future.

#### TYPESCRIPT
We use [TypeScript](https://www.typescriptlang.org/) to create the service and its API REST.


## DATABASE
Don't worry, for now we do not have a database but we use control structures of the language.

## CONTINUOS INTEGRATION (CI)
#### GITLAB

We use a gitlab repository for the project and to configure the CI.
The CI configuration is inside the project in the *.gitlab-ci.yaml* file.

## DEVELOPMENT ENVIRONMENT SETUP

First before starting the project, make sure you have NodeJS installed by covering the official [installation guide](https://nodejs.org/en/download/) on your machine.
...
* To check and fix linting errors run `npm run lint:fix`.
* It is recommended to have the following watchers running:
   - `npm run watch:test`: so that the tests are executed automatically upon any change in the code.
   - `npm run watch:ts` so that it is being automatically transpiled upon any change in the code.
* To do the build, fix linting errors and run the tests you can use `npm run localChecks`.

- Version: 1.0
- Autores: Alvarez Imanol
- Fecha: 29-11-2022
...
A company provide the service of taking people from point A to point B.
...
You can ignore the Gitlab warning Challenge has exceeded its pipeline 
...
In the company, we really appreciate your interest and your time. We are highly 
